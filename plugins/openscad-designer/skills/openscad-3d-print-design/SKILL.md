...
- **Tight/friction** (assembles by hand and stays put — split-part seams, snap-on lids, glued-or-not add-ons): gap of **0.1–0.2mm per side**. Below ~0.1mm, ordinary printer tolerance eats the whole gap and the joint won't go together.
...
Give the gap an explicit named variable (e.g. `connector_gap = 0.15;`) applied symmetrically to both peg and socket — one number to reason about and tune, not something implicit in two independently-chosen diameters.
